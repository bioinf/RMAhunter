...
var init = function(e){
    ...
    tables = counts.map(function(count, i){
        ...
        t.load = function(){
            ...
            if (local_data) {
                t.classList.add('local')
                var x = make_table_rows(local_data.table[i], i);
                if (x.length != 0) {
...
if (local_data) {
    ...
    init(JSON.stringify(['data/_e' + (local_data.samples.indexOf(local_data.current)), local_data.counts]));
}
